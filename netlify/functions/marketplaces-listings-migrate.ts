...
async function refreshAccessToken({ client_id, client_secret, refresh_token, scopes, environment = 'production' }: {
  client_id: string; client_secret: string; refresh_token: string; scopes?: string; environment?: 'sandbox' | 'production';
}) {
  ...
  try {
    ...
    const resp = await fetch(endpoint, { method: 'POST', headers: { Authorization: `Basic ${basic}`, 'Content-Type': 'application/x-www-form-urlencoded' }, body: body.toString() });
    const text = await resp.text();
    if (!resp.ok) return null;
    const json = parseJson(text);
    return json?.access_token ? { access_token: json.access_token, expires_in: json.expires_in, token_type: json.token_type, scope: json.scope } : null;
  } catch { return null; }
}
...
export const handler = async (event: any): Promise<NetlifyResponse> => {
  ...
  try {
    ...
    const { data: account, error: accErr } = await supabaseService.from('marketplace_accounts').select('*')
      .eq('id', account_id).eq('provider', 'ebay').eq('is_active', true).maybeSingle();
    if (accErr || !account)
      return { statusCode: 404, headers: JSON_HEADERS, body: JSON.stringify({ error: 'account_not_found' }) };

    const { data: tokenRows, error: tokErr } = await supabaseService.from('oauth_tokens').select('*')
      .eq('marketplace_account_id', account_id).neq('access_token', 'pending')
      .order('updated_at', { ascending: false }).order('created_at', { ascending: false }).range(0, 0);
    ...
    if (dry_run) {
      ...
      return { statusCode: 200, headers: JSON_HEADERS, body: JSON.stringify({
        dry_run: true, listing_count: listingIds.length, batch_size, batches: Math.min(plannedBatches, max_batches ?? plannedBatches),
    ...
    const ensureRefreshedToken = async (): Promise<string | null> => {
      ...
      let clientId: string = account.client_id || ''; let clientSecret: string = account.client_secret || '';
      if (!clientId || !clientSecret) {
        const { data: credentials } = await supabaseService.from('provider_app_credentials')
          .select('*').eq('provider', 'ebay').eq('environment', account.environment === 'sandbox' ? 'sandbox' : 'production').maybeSingle();
        if (credentials) {
          try {
      ...
      const updateData: any = { access_token: refreshed.access_token, updated_at: new Date().toISOString() };
      if (refreshed.expires_in) updateData.expires_at = new Date(Date.now() + refreshed.expires_in * 1000).toISOString();
    ...
    for (let i = 0; i < listingIds.length; i += batch_size) {
      ...
      const doCall = async () =>
        fetchWithRetry(async () => fetch(migrateUrl, {
          method: 'POST',
          headers: authHeaders(accessToken),
          body: JSON.stringify({ requests: batch.map((id: string) => ({ listingId: id })) })
        }));

      let resp = await doCall(); let raw = await readText(resp);

      if (resp.status === 401) {
        const newToken = await ensureRefreshedToken();
        if (!newToken) {
          for (const lid of batch) results.push({ listingId: lid, status: 'FAILED', errors: [{ message: 'token_expired' }] });
          continue;
        }
        accessToken = newToken; resp = await doCall(); raw = await readText(resp);
      }

      if (!resp.ok) {
        const js = parseJson(raw);
        const errorObj = js?.errors || [{ message: raw?.substring(0, 200) || 'unknown_error' }];
        for (const lid of batch) results.push({ listingId: lid, status: 'FAILED', errors: Array.isArray(errorObj) ? errorObj : [errorObj] });
      } else {
        const js = parseJson(raw);
        ...
        } else for (const lid of batch) results.push({ listingId: lid, status: 'SUCCESS' });
      }
    ...
    await supabaseService.from('sync_logs').insert({
      marketplace_account_id: account_id, operation: 'inventory_migrate',
      outcome: failed === 0 ? 'ok' : (migrated > 0 ? 'retry' : 'fail'),
      http_status: 200, error_message: failed > 0 ? `${failed} failed` : null,
      metadata: { migrated, failed, total: results.length, batchesProcessed }
    });

    return { statusCode: 200, headers: JSON_HEADERS, body: JSON.stringify({
      migrated, failed, total: results.length, batch_size, batches_processed: batchesProcessed, results
    })};
