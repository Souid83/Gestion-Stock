...
export async function syncEbayForProductsFromEbayStock(
  ...
  try {
    ...
    for (const accId of accountIds) {
      ...
      for (let i = 0; i < items.length; i += 100) {
        ...
        try {
          ...
          if (!resp.ok) {
            const t = await resp.text().catch(() => '');
            console.warn('syncEbayForProductsFromEbayStock: stock-update failed', accId, resp.status, t.substring(0, 200));
          }
        } catch (e) {
    ...
    return { success: true, pushed: pushedSkus.size };
  } catch (e: any) {
