...
// Helper to ensure document type exists (upsert by label)
export const ensureDocumentType = async (label: string): Promise<string | null> => {
  console.log('ensureDocumentType: checking/creating type:', label);
  ...
  if (existing) {
    console.log('ensureDocumentType: type exists with id:', existing.id);
    return existing.id;
  }
  ...
  if (createError) {
    console.error('ensureDocumentType: error creating type:', createError);
    return null;
  }

  console.log('ensureDocumentType: type created with id:', created.id);
  return created.id;
};

// Helper to get all active document types
export const getDocumentTypes = async (): Promise<DocumentType[]> => {
  try {
    console.log('getDocumentTypes: fetching document types...');
    ...
    if (error) {
      console.error('getDocumentTypes: error fetching types:', error);

      // Check if table exists error
      if (error.message?.includes('does not exist') || error.code === '42P01') {
        console.error('getDocumentTypes: billing_document_types table does not exist!');
      }
    ...
    if (!data || data.length === 0) {
      console.warn('getDocumentTypes: no document types found in database');
      return [];
    }

    console.log(`getDocumentTypes: successfully fetched ${data.length} types:`, data);
    return data;
  } catch (err) {
    console.error('getDocumentTypes: unexpected error:', err);
    return [];
  }
};
    ...
    try {
      ...
      const { data: orderData, error: orderError } = await supabase
        ...
        .insert([{
          ...
          document_type_id: quote.document_type_id
        }])
        .select()
        .single();
