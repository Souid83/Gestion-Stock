...
export const useProductStore = create<ProductStore>((set, get) => ({
  ...
  fetchProducts: async () => {
    ...
    try {
      ...
      // Regrouper les parent_id pour une récupération en batch
      const parentIds = Array.from(
        new Set(
          rows.map((p: any) => p.parent_id).filter((v: any) => !!v)
        )
      );
      ...
      // Si produit enfant → hérite du stock et stock_total du parent
      const productsWithStocks = rows.map((p: any) => {
        if (p.parent_id) {
          const parent = parentsById[p.parent_id];
          return {
            ...p,
  ...
  addProduct: async (product: ProductInsert) => {
    ...
    try {
      ...
      // Si parent_id est fourni → création d'un miroir
  ...
  updateProduct: async (id: string, updates: Partial<Product>) => {
    ...
    try {
      ...
      if (existingAny.parent_id) {
        // Enfant miroir: seul 'name' est modifiable
