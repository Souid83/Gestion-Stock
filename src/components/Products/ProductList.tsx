...
import { Edit, Trash2, AlertCircle, Image as ImageIcon, Plus, Package, Eye, ChevronDown, ChevronUp, Battery, Users, ExternalLink, PackagePlus } from 'lucide-react';
...
export const ProductList: React.FC<ProductListProps> = ({ products: initialProducts, lots = [] }) => {
  ...
  const canCreateLot = (product: Product): boolean => {
    // Lots uniquement pour produits à prix d'achat unique
    return !product.parent_id && !product.serial_number;
  };
                            ...
                            if (children.length === 0) {
                              ...
                              if (product.vat_type === "margin") {
                                ...
                                return (
                                  <span className="flex flex-col">
                                    <span>
                                      <span className="font-medium">{(proPrice * 1.2).toFixed(2)} € TTC</span>
                                      <span className={`ml-1 ${color}`}>({proMarginPercent.toFixed(1)}%)</span>
                                    </span>
                                    <span className="text-xs text-gray-500">
                                  ...
                                  if (tva === "TTC") {
                                    ...
                                    return (
                                      <span key={tva} className="flex flex-col">
                                        <span>
                                          <span className="font-medium">{(prix * 1.2).toFixed(2)} € TTC</span>
                                          <span className={`ml-1 ${color}`}>({marge.toFixed(1)}%)</span>
                                        </span>
                                        <span className="text-xs text-gray-500">
                                          {prix.toFixed(2)} € HT
                            ...
                            if (children.length === 0) {
                              ...
                              if (product.vat_type === "margin") {
                                ...
                                return (
                                  <span className="flex flex-col">
                                    <span>
                                      <span className="font-medium">{(retailPrice * 1.2).toFixed(2)} € TTC</span>
                                      <span className={`ml-1 ${color}`}>({retailMarginPercent.toFixed(1)}%)</span>
                                    </span>
                                    <span className="text-xs text-gray-500">
                              ...
                              return (
                                <div className="flex flex-col gap-1">
                                  <span className="flex items-center gap-2">
                                    ...
                                    <span className="text-gray-900">{normalCount} TTC</span>
                                  </span>
                                  <span className="flex items-center gap-2">
                                    <Package size={16} className={iconColorSer} />
                                    <span className="text-gray-900">{marginCount} TVM</span>
                                  </span>
                                </div>
                              );
                            }
                                        ...
                                        {(() => {
                                          ...
                                          if (child.vat_type === "margin") {
                                            ...
                                            return (
                                              <span className="flex flex-col">
                                                <span>
                                                  <span className="font-medium">{prixTTC.toFixed(2)} € TTC</span>
                                                  <span className={`ml-1 ${color}`}>({marge.toFixed(1)}%)</span>
                                                </span>
                                                <span className="text-xs text-gray-500">
                                                  {prixHT.toFixed(2)} € HT
